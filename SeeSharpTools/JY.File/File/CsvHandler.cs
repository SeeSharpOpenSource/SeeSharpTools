...
//            return FileUtil.GetStrData(fileDatas, Delims);
        ...
        public static void WriteData(string filePath, string[] data, WriteMode writeMode = WriteMode.OverLap)
        {
            FileUtil.CheckFilePath(filePath, FileExtName);
            //一维数组需要被转换为1列n行的形式
            StreamWriteData(filePath, data.GetEnumerator(), data.Length, 1, writeMode);
        }
        ...
        {
            FileUtil.CheckFilePath(filePath, FileExtName);
            StreamWriteData(filePath, data.GetEnumerator(), data.GetLength(0), data.GetLength(1), writeMode);
        }
        ...
        ///     </param>
        /// </summary>
        public static void WriteData(string filePath, float[,] data, WriteMode writeMode = WriteMode.OverLap)
        {
            FileUtil.CheckFilePath(filePath, FileExtName);
            StreamWriteData(filePath, data.GetEnumerator(), data.GetLength(0), data.GetLength(1), writeMode);
        }
        ...
        ///     </param>
        /// </summary>
        public static void WriteData(float[] data, WriteMode writeMode = WriteMode.OverLap)
        {
            string filePath = FileUtil.GetSaveFilePathFromDialog(FileExtName);
            WriteData(filePath, data, writeMode);
        }
        ...
        private static void StreamWriteData(string filePath, IEnumerator enumerator,
            int rowCount, int colCount, WriteMode writeMode)
        {
            StreamWriter writer = null;
            try
            {
                FileUtil.InitWriteStream(ref writer, filePath, writeMode);
