...
{
    public class MultiWaveData<XDataType, YDataType>
    {
        ...
        public MultiWaveData(XDataType[] xData, YDataType[,] yData, bool fullCopy = false)
        {
            CheckData(xData, yData);

            SampleSize = xData.Length;
            ChannelCount = yData.GetLength(0);

            if (fullCopy)
            {
                FullCopyXData(xData);
                int yTypeSize = Marshal.SizeOf(typeof(YDataType));
                _yDataCache = new YDataType[yData.GetLength(0), yData.GetLength(1)];
