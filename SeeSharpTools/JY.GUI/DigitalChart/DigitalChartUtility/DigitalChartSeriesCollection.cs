...
{
    public class DigitalChartSeriesCollection : IList<DigitalChartSeries>
    {
        private readonly List<DigitalChartSeries> _lineSeries;
        ...
        internal DigitalChartSeriesCollection(SeriesCollection chartSeries, DigitalChart parentChart)
        {
            this._plotSeries = chartSeries;
            _lineSeries = new List<DigitalChartSeries>(Constants.DefaultMaxSeriesCount);
            this._parentChart = parentChart;
            // 默认添加n个Series
            foreach (Series plotSeries in _plotSeries)
            {
                DigitalChartSeries series = new DigitalChartSeries();
        ...
        public void AdaptSeriesCount(int plotSeriesCount)
        {
            ...
            {
                AddInternal(new DigitalChartSeries());
            }
        ...
        IEnumerator<DigitalChartSeries> IEnumerable<DigitalChartSeries>.GetEnumerator()
        {
        ...
        public void CopyTo(DigitalChartSeries[] array, int index)
        {
            return;
        }
        ...
        public void Add(DigitalChartSeries item)
        {
...
//                _designStartFlag = true;
        ...
        internal void AddInternal(DigitalChartSeries item)
        {
            // 开始内部Add后即设计时代码已结束
            _designEndFlag = true;
...
//        public bool Contains(object value)
        public bool Contains(DigitalChartSeries item)
        {
            if (null == item)
            {
                return false;
...
//            }
        ...
        public int IndexOf(DigitalChartSeries item)
        {
            if (null == item)
            {
                return -1;
        ...
        public void Insert(int index, DigitalChartSeries item)
        {
...
//        public void Remove(object value)
        public bool Remove(DigitalChartSeries item)
        {
        ...
        {
            DigitalChartSeries item = value as DigitalChartSeries;
            int index = _lineSeries.IndexOf(item);
            // LineSeries个数不能少于PlotSeries的个数
            if (_plotSeries.Count > _lineSeries.Count - 1 || null == item || index < 0)
            {
...
//        public object this[int index]
        public DigitalChartSeries this[int index]
        {
        ...
        internal void RefreshPlotAxis(DigitalChartSeries series)
        {
            bool isSplitView = _parentChart.SplitView;
            if (null == series)
            {
                foreach (DigitalChartSeries xSeries in _lineSeries)
                {
