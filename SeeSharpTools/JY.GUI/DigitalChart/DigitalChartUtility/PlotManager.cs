...
namespace SeeSharpTools.JY.GUI.DigitalChartUtility
{
    internal class PlotManager
    {
        ...
        private readonly DigitalChartSeriesCollection _series;

        internal DataCheckParameters DataCheckParams;

        private DigitalChart.FitType _fitType;
        public DigitalChart.FitType FitType
        {
            get
            {
                return _fitType;
        ...
        public DigitalChartSeriesCollection Series => _series;

        public DigitalChartLineSeries LineSeries { get; }
        ...
        internal PlotManager(DigitalChart parentChart, SeriesCollection plotSeries)
        {

            // LineSeries只是一个用于维护对外接口的属性
            this._fitType = DigitalChart.FitType.Range;
            ...
            this._series = new DigitalChartSeriesCollection(plotSeries, parentChart);
            LineSeries = new DigitalChartLineSeries(_series);
        ...
        internal void AdaptSeriesCount()
        {
            ...
            if (_plotSeriesCount > PlotSeries.Count)
            {
                for (int i = PlotSeries.Count; i < _plotSeriesCount; i++)
                {
                    ...
                    ((DigitalChartSeries)_series[i]).AdaptBaseSeries(newSeries);
...
//        }
        ...
        public void GetMaxAndMinYValue(DigitalChartPlotArea plotArea, out double maxYValue, out double minYValue, 
            int seriesIndex)
        {
        ...
        public void GetMaxAndMinY2Value(DigitalChartPlotArea plotArea, out double maxYValue, out double minYValue, 
            int seriesIndex)
        {
            maxYValue = double.NaN;
            minYValue = double.NaN;
            // 副坐标轴没有数据则返回
            if (!HasSeriesInYAxis(plotArea, DigitalChartAxis.PlotAxis.Secondary))
            {
                return;
            }
            double tmpMaxYValue, tmpMinYValue;
        ...
        public bool HasSeriesInXAxis(DigitalChartAxis.PlotAxis plotAxis, DigitalChartPlotArea plotArea)
        {
            return PlotSeries.Any(item => item.ChartArea.Equals(plotArea.Name) && item.XAxisType == (AxisType) plotAxis);
        }

        public bool HasSeriesInYAxis(DigitalChartPlotArea plotArea, DigitalChartAxis.PlotAxis plotAxis)
        {
