...
{
    public class DigitalChartTabCursorCollection : IList<DigitalChartTabCursor>
    {
        private readonly DigitalChart _parentChart;
        private readonly DigitalChartPlotArea _parentPlotArea;
        private readonly Chart _baseChart;
        private IList<DigitalChartTabCursor> _cursors;
        ...
        internal DigitalChartTabCursorCollection(DigitalChart parentChart, Chart baseChart, DigitalChartPlotArea parentPlotArea)
        {
            ...
            this._cursors = new List<DigitalChartTabCursor>(MaxCursorCount);
...
//            this._baseChart.PostPaint += BaseChartOnPostPaint;
        ...
        public IEnumerator<DigitalChartTabCursor> GetEnumerator()
        {
        ...
        public void Add(DigitalChartTabCursor item)
        {
        ...
        public void Clear()
        {
            ...
            {
                if (_baseChart.Controls[i] is DigitalChartTabCursorControl)
                {
        ...
        public bool Contains(DigitalChartTabCursor item)
        {
            return _cursors.Contains(item);
        }

        public void CopyTo(DigitalChartTabCursor[] array, int arrayIndex)
        {
            throw new NotSupportedException();
        }

        public bool Remove(DigitalChartTabCursor item)
        {
            for (int i = _baseChart.Controls.Count - 1; i >= 0; i--)
            {
        ...
        public int IndexOf(DigitalChartTabCursor item)
        {
            return _cursors.IndexOf(item);
        }

        public void Insert(int index, DigitalChartTabCursor item)
        {
            // TODO other task
            if (_cursors.Any(cursor => cursor.Name.Equals(item.Name)) || _cursors.Count >= MaxCursorCount)
            {
        ...
        public DigitalChartTabCursor this[int index]
        {
        ...
        private void BaseChartOnPostPaint(object sender, ChartPaintEventArgs eventArgs)
        {
            ...
            {
                foreach (DigitalChartTabCursor cursor in _cursors)
                {
        ...
        public void RefreshCursorPosition(DigitalChartTabCursor cursor)
        {
        ...
        internal void RefreshCursorValue(DigitalChartTabCursor cursor)
        {
            _adapter.RefreshCursorValue(cursor);
        }

        internal void ShowCursorValue(DigitalChartTabCursor cursor, bool isShow)
        {
        ...
        private void SetCursorXBoundry(DigitalChartTabCursor item)
        {
            int minXBound = (int)Math.Round(_adapter.PlotRealX);
            int maxXBound = (int)Math.Round(_adapter.PlotRealX + _adapter.PlotRealWidth);
