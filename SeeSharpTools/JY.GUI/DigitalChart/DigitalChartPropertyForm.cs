...
{
    public partial class DigitalChartPropertyForm : Form
    {
        private DigitalChart _changedCtrl;
        private Control _beforeCtrl;
        //标志位，判断是由于调用this.close()完成的窗口关闭(true)，还是由于鼠标按下导致的窗口关闭(false)
        private bool windowCloseButton;

        public DigitalChartPropertyForm(DigitalChart _ctrl)
        {
            InitializeComponent();
...
//            checkBox_YAxisLogarithmic.Checked = _changedCtrl.YAxisLogarithmic;
        ...
        {
            _changedCtrl.GradientStyle =  (DigitalChart.ChartGradientStyle)Enum.Parse(typeof(DigitalChart.ChartGradientStyle), comboBox_GradientStyle.Text, true);
        }
...
//            _changedCtrl.XAxisLogarithmic = checkBox_XAxisLogarithmic.Checked;
        ...
        {
            DigitalChart beforeDigitalChart = ((DigitalChart)_beforeCtrl);
            _changedCtrl.Size = beforeDigitalChart.Size;

            _changedCtrl.LegendVisible = beforeDigitalChart.LegendVisible;
            _changedCtrl.YAutoEnable = beforeDigitalChart.YAutoEnable;
            double yAxisRange = beforeDigitalChart.AxisY.Maximum;
            if (!double.IsNaN(yAxisRange))
            {
                _changedCtrl.AxisY.Maximum = (int)yAxisRange;
            }
            yAxisRange = beforeDigitalChart.AxisY.Minimum;
            if (!double.IsNaN(yAxisRange))
            {
                _changedCtrl.AxisY.Minimum = (int)yAxisRange;
            }

            _changedCtrl.BackColor = beforeDigitalChart.BackColor;
            _changedCtrl.ChartAreaBackColor = beforeDigitalChart.ChartAreaBackColor;
            _changedCtrl.LegendBackColor = beforeDigitalChart.LegendBackColor;
            _changedCtrl.GradientStyle = beforeDigitalChart.GradientStyle;
...
//            _changedCtrl.YAxisLogarithmic = beforeEasyChart.YAxisLogarithmic;
        ...
        private void SetAxisValue(DigitalChartAxis axis, double max, double min)
        {
            if (max > axis.Minimum)
            {
                axis.Maximum = max;
